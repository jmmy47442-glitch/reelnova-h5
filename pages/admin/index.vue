...
import { ArrowDownRight, ArrowUpRight, ChevronRight, CircleDollarSign, CloudOff, Download, Eye, Film, Plus, RefreshCw, ShoppingCart, TriangleAlert } from 'lucide-vue-next';
...
const metrics = computed(() => data.value ? [
  { label: '今日播放量', value: data.value.metrics.plays.value.toLocaleString(), metric: data.value.metrics.plays, icon: Eye, color: 'blue', to: '/admin/metrics/plays' },
  { label: '今日订单', value: data.value.metrics.orders.value.toLocaleString(), metric: data.value.metrics.orders, icon: ShoppingCart, color: 'green', to: '/admin/metrics/orders' },
  { label: '支付成功金额', value: `$${data.value.metrics.revenue.value.toFixed(2)}`, metric: data.value.metrics.revenue, icon: CircleDollarSign, color: 'orange', to: '/admin/metrics/revenue' },
  { label: '异常订单', value: data.value.metrics.exceptions.value.toLocaleString(), metric: data.value.metrics.exceptions, icon: TriangleAlert, color: 'purple', to: '/admin/metrics/exceptions' },
] : []);
...
const downloadReport = () => {
  ...
  downloadCsv('reelnova-dashboard-real.csv', ['日期', '播放量', '支付成功金额(USD)'], data.value.trends.map((point) => [point.date, point.plays, point.revenue.toFixed(2)]));
...
<template>
  <div>
    ...
    <template v-else>
      ...
      <section class="admin-metric-grid"><NuxtLink v-for="item in metrics" :key="item.label" :to="item.to" class="admin-panel admin-metric" :aria-label="`查看${item.label}明细`"><div class="admin-metric__top"><span class="admin-metric__icon" :class="`is-${item.color}`"><component :is="item.icon" :size="21" /></span><span class="admin-metric__change" :class="item.metric.changePercent !== null && item.metric.changePercent < 0 ? 'down' : 'up'"><ArrowDownRight v-if="item.metric.changePercent !== null && item.metric.changePercent < 0" :size="14" /><ArrowUpRight v-else :size="14" />{{ formatChange(item.metric.changePercent) }}</span></div><span>{{ item.label }}</span><strong>{{ item.value }}</strong><small><span>昨日同期 {{ item.metric.previousValue.toLocaleString() }}</span><span class="admin-metric__link">查看明细 <ChevronRight :size="13" /></span></small></NuxtLink></section>
