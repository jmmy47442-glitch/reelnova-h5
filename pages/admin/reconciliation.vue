...
const { data, status, error, refresh } = useLazyAsyncData('admin-reconciliation-real', () => api.getReconciliation(days.value, environment.value), { watch: [days, environment] });
...
const exportReport = () => {
  downloadCsv(`reelnova-reconciliation-${environment.value}-${days.value}d-real.csv`, ['日期', '币种', '成功金额', '手续费', '退款金额', '净额', '成功支付数', '异常订单'], rows.value.map((row) => [row.date, row.currency, row.gross.toFixed(2), row.fee.toFixed(2), row.refunds.toFixed(2), row.net.toFixed(2), row.paid, row.exceptions]));
...
<template>
  <div>
    <AdminPageHeader title="对账中心" description="按 UTC 支付确认日和退款完成日统计；正式收款与沙箱测试分开核算。"><el-button :disabled="!data" @click="exportReport"><Download :size="16" />导出对账 CSV</el-button></AdminPageHeader>
    ...
    <template v-else>
      ...
      <div class="reconciliation-grid"><section class="admin-panel reconciliation-chart"><div class="admin-panel__header"><div><h2>每日成功金额</h2><p>包含已退款订单的原始收款，退款单独扣减</p></div><el-tag type="success">{{ totals.paid.toLocaleString() }} 笔成功支付</el-tag></div><div v-if="rows.length" class="revenue-bars"><div v-for="row in rows" :key="`${row.date}-${row.currency}`" class="revenue-bar"><strong>${{ row.gross.toFixed(0) }}</strong><span :style="{ height: `${Math.max(2, Math.round(row.gross / maxGross * 100))}%` }" /><small>{{ row.date.slice(5) }}</small></div></div><div v-else class="table-empty"><ReceiptText :size="28" /><span>该时段暂无已确认支付</span></div></section><aside class="admin-panel exception-panel"><div class="admin-panel__header"><div><h2>对账异常</h2><p>支付失败或风控审核</p></div><AlertTriangle :size="20" /></div><div class="exception-count"><strong>{{ totals.exceptions }}</strong><span>{{ period }}异常订单</span></div><p class="exception-note">金额或币种不一致的订单会进入风控审核，不计入收入且不发放权益。</p><el-button type="primary" plain @click="navigateTo('/admin/orders')">查看订单核验</el-button></aside></div>
