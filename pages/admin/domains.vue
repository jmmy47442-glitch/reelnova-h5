...
import { CheckCircle2, Copy, Globe2, Plus, RefreshCw, Settings2, ShieldCheck, Trash2 } from 'lucide-vue-next';
...
const form = reactive({ host: 'iseedrama.com' });
...
const missingFieldLabels: Record<'zoneId' | 'apiToken' | 'cnameTarget', string> = {
  zoneId: 'Zone ID', apiToken: 'API Token（服务端 Secret）', cnameTarget: 'CNAME 接入目标',
};
...
const toggleRedirect = async (domain: DomainConfig) => {
  ...
  } finally { redirecting.value = ''; }
};

const cloudflareStatusLabels: Record<string, string> = {
  active: '路由已激活', pending: '等待验证', moved: '已迁移', blocked: '已阻止', deleted: '已删除',
};
...
<template>
  <div>
    <AdminPageHeader title="域名管理" description="通过 Cloudflare 管理自定义域名、证书签发、主域名与旧域名 301。"><el-button @click="settingsVisible = true"><Settings2 :size="16" />接入设置</el-button><el-button type="primary" :disabled="!automationConfigured" @click="dialogVisible = true"><Plus :size="16" />绑定域名</el-button></AdminPageHeader>
    <el-alert v-if="!automationConfigured && !loading" :title="missingConfigurationText" type="warning" show-icon :closable="false" class="admin-alert"><template #default><span>Zone ID 与 CNAME 目标可在接入设置中维护；API Token 为敏感凭据，只能通过服务端 Secret 配置。</span><el-button link type="primary" @click="settingsVisible = true">打开接入设置</el-button></template></el-alert>
    ...
    <el-empty v-else-if="!state.domains.length" description="尚未绑定自定义域名"><el-button type="primary" :disabled="!automationConfigured" @click="dialogVisible = true">绑定域名</el-button></el-empty>
    <section v-else class="domain-grid"><article v-for="domain in state.domains" :key="domain.id" class="admin-panel domain-card"><div class="domain-card__header"><span class="domain-icon"><Globe2 :size="20" /></span><div><strong>{{ domain.host }}</strong><span>Custom Hostname {{ domain.cloudflareHostnameId || '未创建' }}</span></div><el-tag :type="domain.role === '主域名' ? 'primary' : 'info'">{{ domain.role }}</el-tag></div><div class="domain-health"><div><span>Cloudflare 路由</span><strong :class="domain.cloudflareStatus === 'active' ? 'success-text' : 'warning-text'">{{ cloudflareStatusLabel(domain.cloudflareStatus) }}</strong></div><div><span>CNAME 验证</span><strong :class="domain.verification === '已验证' ? 'success-text' : 'warning-text'"><CheckCircle2 v-if="domain.verification === '已验证'" :size="15" />{{ domain.verification }}</strong></div><div><span>HTTPS 证书</span><strong :class="domain.certificate === '正常' ? 'success-text' : 'warning-text'"><ShieldCheck :size="15" />{{ domain.certificate }}</strong></div><div><span>301 到主域名</span><el-switch v-model="domain.redirect" :loading="redirecting === domain.id" :disabled="domain.role === '主域名'" @change="toggleRedirect(domain)" /></div></div><p v-if="domain.provisioningError" class="domain-error">{{ domain.provisioningError }}</p><div class="domain-card__footer"><span>更新于 {{ new Date(domain.updatedAt).toLocaleString('zh-CN', { hour12: false }) }}</span><div><el-button link @click="showDns(domain)">DNS 配置</el-button><el-button link type="primary" :loading="verifying === domain.id" @click="verify(domain)"><RefreshCw :size="14" />同步状态</el-button><el-button v-if="domain.verification === '已验证' && domain.certificate === '正常' && domain.role !== '主域名'" link type="primary" @click="setPrimary(domain)">设为主域名</el-button><el-button v-if="domain.role !== '主域名'" link type="danger" :loading="deleting === domain.id" title="解除域名绑定" @click="removeDomain(domain)"><Trash2 :size="14" /></el-button></div></div></article></section>
    <el-dialog v-model="dialogVisible" title="绑定自定义域名" width="min(500px, 92vw)"><el-form label-position="top"><el-form-item label="域名" required><el-input v-model="form.host" placeholder="iseedrama.com" /></el-form-item><el-alert title="确认后会立即创建 Cloudflare Custom Hostname 并申请 DV 证书；验证通过前只作为备用域名。" type="info" :closable="false" /></el-form><template #footer><el-button @click="dialogVisible = false">取消</el-button><el-button type="primary" :loading="adding" @click="addDomain">创建绑定</el-button></template></el-dialog>
    <el-dialog v-model="settingsVisible" title="域名接入设置" width="min(560px, 92vw)"><el-form label-position="top"><el-form-item label="Cloudflare Zone ID" required><el-input v-model="domainSettings.zoneId" maxlength="32" placeholder="32 位 Zone ID" /></el-form-item><el-form-item label="CNAME 接入目标" required><el-input v-model="domainSettings.cnameTarget" placeholder="customers.example.com" /><div class="domain-setting-help">用户域名需要 CNAME 指向此主机名，不包含协议和路径。</div></el-form-item><div class="domain-secret-status"><span>Cloudflare API Token</span><el-tag :type="domainSettings.apiTokenConfigured ? 'success' : 'warning'">{{ domainSettings.apiTokenConfigured ? '服务端已配置' : '服务端未配置' }}</el-tag><p>Token 不在后台显示或保存，请通过部署环境的 <code>CLOUDFLARE_API_TOKEN</code> Secret 管理。</p></div></el-form><template #footer><el-button @click="settingsVisible = false">取消</el-button><el-button type="primary" :loading="savingSettings" @click="saveSettings">保存并验证</el-button></template></el-dialog>
    <el-drawer v-model="dnsVisible" title="DNS 与证书验证" size="min(560px, 92vw)"><template v-if="activeDomain"><el-alert title="在域名服务商处添加 CNAME；Cloudflare 返回 TXT 记录时也必须一并添加。" type="info" :closable="false" show-icon /><h3 class="dns-section-title">流量接入</h3><div class="dns-record"><span>记录名称</span><strong>{{ activeDomain.host }}</strong><button type="button" title="复制" @click="copy(activeDomain.host)"><Copy :size="15" /></button></div><div class="dns-record"><span>记录类型</span><strong>CNAME</strong><button type="button" title="复制" @click="copy('CNAME')"><Copy :size="15" /></button></div><div class="dns-record"><span>记录值</span><strong>{{ cnameTarget }}</strong><button type="button" title="复制" @click="copy(cnameTarget)"><Copy :size="15" /></button></div><template v-if="activeDomain.validationRecords?.length"><h3 class="dns-section-title">证书验证</h3><template v-for="record in activeDomain.validationRecords" :key="`${record.name}:${record.value}`"><div class="dns-record"><span>{{ record.type }} 名称</span><strong>{{ record.name }}</strong><button type="button" title="复制" @click="copy(record.name)"><Copy :size="15" /></button></div><div class="dns-record"><span>{{ record.type }} 值</span><strong>{{ record.value }}</strong><button type="button" title="复制" @click="copy(record.value)"><Copy :size="15" /></button></div></template></template><el-button type="primary" :loading="verifying === activeDomain.id" @click="verify(activeDomain)"><RefreshCw :size="15" />同步 Cloudflare 状态</el-button></template></el-drawer>
  </div>
</template>
