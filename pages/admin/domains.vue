...
<template>
  <div>
    ...
    <el-dialog v-model="dialogVisible" title="绑定自定义域名" width="min(500px, 92vw)"><el-form label-position="top"><el-form-item label="域名" required><el-input v-model="form.host" placeholder="watch.example.com" /></el-form-item><el-alert title="确认后会立即创建 Cloudflare Custom Hostname 并申请 DV 证书；验证通过前只作为备用域名。" type="info" :closable="false" /></el-form><template #footer><el-button @click="dialogVisible = false">取消</el-button><el-button type="primary" :loading="adding" @click="addDomain">创建绑定</el-button></template></el-dialog>
