...
const exportOrders = () => {
  downloadCsv('reelnova-orders-real.csv', ['本地订单号', '剧名', '用户', '国家', '金额', '币种', '支付方式', '手续费', '净额', '支付状态', '退款状态', '权益状态', 'PayPal Order ID', 'Capture ID', 'PayPal Refund ID', '创建时间', '回调时间'], rows.value.map((order) => [order.orderNo, order.seriesTitle, order.email || '', order.country || '', order.amount.toFixed(2), order.currency, paymentMethodLabel(order.paymentMethod), order.fee.toFixed(2), order.netAmount.toFixed(2), labels[order.status], order.refund.status ? refundLabels[order.refund.status] : '无退款', entitlementLabels[order.entitlement], order.paypalOrderId || '', order.captureId || '', order.refund.paypalRefundId || '', order.createdAt, order.callbackAt || '']));
