...
const customHostnamesStatusText = computed(() => {
  if (!data.value?.cloudflare.cloudflareForSaasEnabled) return '待 Cloudflare for SaaS 开通（不阻塞 MVP）';
...
<template>
  <div>
    ...
    <div class="connection-grid">
      ...
      <section class="admin-panel connection-panel"><div class="admin-panel__header"><div><h2>Cloudflare 媒体与域名</h2><p>R2 原片、Container FFmpeg 转码与签名 HLS 播放</p></div><el-tag :type="data?.cloudflare.mediaConfigured ? 'success' : 'warning'">{{ data?.cloudflare.mediaConfigured ? '已配置' : '待配置' }}</el-tag></div><div class="connection-feature"><Video :size="23" /><div><strong>上传、转码与签名播放</strong><span>原片分片写入私有 R2，Cloudflare Container 生成多清晰度 HLS，播放前由服务端校验权益。</span></div></div><div class="connection-check"><span>R2 上传 Worker</span><strong>{{ data?.cloudflare.mediaWorkerConfigured ? '已配置' : '待配置' }}</strong></div><div class="connection-check"><span>R2 连接</span><strong :class="data?.cloudflare.mediaWorkerReady ? 'success-text' : 'danger-text'">{{ data?.cloudflare.mediaWorkerReady ? '已连通' : data?.cloudflare.mediaWorkerError || '待配置' }}</strong></div><div class="connection-check"><span>FFmpeg Container</span><strong :class="data?.cloudflare.transcoderReady ? 'success-text' : 'danger-text'">{{ data?.cloudflare.transcoderReady ? '已连通' : data?.cloudflare.transcoderError || '待配置' }}</strong></div><div class="connection-check"><span>输入格式</span><strong>MP4 · MOV · MKV · WebM · AVI · MPEG</strong></div><div class="connection-check"><span>动态备用域名</span><strong>{{ customHostnamesStatusText }}</strong></div><div class="connection-check"><span>播放跟踪签名</span><strong>{{ data?.cloudflare.mediaSigningConfigured ? '已加密配置' : '待配置' }}</strong></div></section>
      <section class="admin-panel connection-panel"><div class="admin-panel__header"><div><h2>上线步骤</h2><p>按当前开通进度继续配置</p></div></div><ol class="deployment-steps"><li><span>1</span><div><strong>确认 D1 已执行全部迁移</strong><code>npm run db:migrate -- --apply</code></div></li><li><span>2</span><div><strong>创建 R2 与媒体 Worker</strong><small>配置私有 R2 binding 和签名密钥</small></div></li><li><span>3</span><div><strong>绑定 3 个自有 Custom Domains</strong><small>Nuxt：根域名与 admin；媒体 Worker：media</small></div></li><li><span>4</span><div><strong>创建 www 永久跳转规则</strong><small>301 到根域名，并保留路径与查询参数</small></div></li></ol><a class="el-button" href="https://dash.cloudflare.com/" target="_blank" rel="noreferrer">打开 Cloudflare<ExternalLink :size="15" /></a></section>
    </div>
