...
<template>
  <div>
    ...
    <div class="connection-grid">
      ...
      <section class="admin-panel connection-panel"><div class="admin-panel__header"><div><h2>Cloudflare 媒体与域名</h2><p>私有 R2 分片上传与签名 MP4 播放</p></div><el-tag :type="data?.cloudflare.mediaConfigured ? 'success' : 'warning'">{{ data?.cloudflare.mediaConfigured ? '已配置' : '待配置' }}</el-tag></div><div class="connection-feature"><Video :size="23" /><div><strong>上传与签名播放</strong><span>兼容 MP4 分片写入私有 R2，播放前由服务端校验权益；其他格式需先在本地转换。</span></div></div><div class="connection-check"><span>R2 上传 Worker</span><strong>{{ data?.cloudflare.mediaWorkerConfigured ? '已配置' : '待配置' }}</strong></div><div class="connection-check"><span>R2 连接</span><strong :class="data?.cloudflare.mediaWorkerReady ? 'success-text' : 'danger-text'">{{ data?.cloudflare.mediaWorkerReady ? '已连通' : data?.cloudflare.mediaWorkerError || '待配置' }}</strong></div><div class="connection-check"><span>播放模式</span><strong>{{ data?.cloudflare.delivery === 'r2-mp4' ? '兼容 MP4 直播放' : '签名 HLS / MP4' }}</strong></div><div class="connection-check"><span>输入格式</span><strong>H.264（8 位）+ AAC-LC MP4</strong></div><div class="connection-check"><span>动态备用域名</span><strong>{{ customHostnamesStatusText }}</strong></div><div class="connection-check"><span>播放跟踪签名</span><strong>{{ data?.cloudflare.mediaSigningConfigured ? '已加密配置' : '待配置' }}</strong></div></section>
