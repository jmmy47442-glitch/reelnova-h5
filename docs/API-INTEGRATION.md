...
Episode uploads use 10 MiB R2 multipart chunks. The browser may resume an unexpired upload session. Completion validates the actual private R2 object (H.264 + AAC-LC, non-fragmented MP4; faststart is recommended) and returns `ready` or `failed` with `errorMessage`; transient failures remain recoverable by the hourly reconciliation job. Validation follows MP4 offsets and reads at most 16 MiB, so metadata may be stored after `mdat`. No Stream subscription or callback is used. `/api/admin/media/:assetId/retry` revalidates the existing object. A series cannot be published until every non-deleted episode has a `ready` media asset. Playback authorization prefers `delivery: "hls"` when a version-matched HLS package is published, returning a signed `master.m3u8` URL and bounded startup `prefetchUrls`; `originalUrl` is also returned when the private source MP4 passes playback validation, allowing source-quality playback with the same expiry. It is omitted if the original is incompatible or unavailable, and for Cloudflare Stream assets. Otherwise it returns `delivery: "mp4"`, with identical `signedUrl` and `originalUrl`. All manifests, init files and segments require a valid token; no Stream service is involved. Cover images are uploaded separately. See [R2 MP4 deployment](./R2-MP4-DELIVERY.md).
