...
Episode uploads use 10 MiB R2 multipart chunks and may resume while the session is valid. The production free-plan mode accepts H.264 8-bit + AAC-LC MP4 inputs up to 20 GB and six hours; other containers/codecs must be converted locally before upload. Compatible files become `ready` immediately. Browser preflight and Media Worker validation both enforce this contract, so bypassing the UI cannot queue an unavailable Container transcode. Transient completion failures remain recoverable by the hourly reconciliation job. Direct MP4 validation follows box offsets and reads at most 16 MiB, so metadata may be stored after `mdat`. `/api/admin/media/:assetId/retry` revalidates the stored MP4. A series cannot be published until every non-deleted episode has a `ready` media asset. Playback authorization continues to serve a version-matched legacy HLS package when one exists; otherwise compatible originals return `delivery: "mp4"`. All manifests, segments and originals require a valid token; Cloudflare Stream is not used. Covers are uploaded separately. Optional paid Container transcoding is documented in [Container transcoding](./CLOUDFLARE-CONTAINER-TRANSCODING.md).
