...
Episode uploads use 10 MiB R2 multipart chunks and may resume while the session is valid. MP4, M4V, MOV, MKV, WebM, AVI and MPEG inputs up to 20 GB are accepted. Compatible H.264 + AAC-LC MP4 files become `ready` immediately; other inputs return `processing` and are converted by a durable Cloudflare Workflow and FFmpeg Container into four-rendition HLS. Transient completion failures remain recoverable by the hourly reconciliation job. Direct MP4 validation follows box offsets and reads at most 16 MiB, so metadata may be stored after `mdat`. `/api/admin/media/:assetId/retry` revalidates direct MP4 or restarts the Container workflow. A series cannot be published until every non-deleted episode has a `ready` media asset. Playback authorization prefers `delivery: "hls"` when a version-matched HLS package is published, returning a signed `master.m3u8` URL and bounded startup `prefetchUrls`; `originalUrl` is also returned when the private source MP4 passes playback validation. Otherwise compatible originals return `delivery: "mp4"`. All manifests, init files, segments and originals require a valid token; Cloudflare Stream is not used. Covers are uploaded separately. See [Container transcoding](./CLOUDFLARE-CONTAINER-TRANSCODING.md) and [R2/HLS delivery](./R2-MP4-DELIVERY.md).
...
`POST /api/orders` accepts `paymentMethod: "paypal" | "card" | "apple_pay"` (default: `"paypal"`). Credit and debit cards both use `"card"` and the H5 **Credit or debit card** checkout. PayPal Card Fields collects the cardholder name, card number, expiration date and security code directly; card details must never be sent to this API. Buyers do not need a PayPal account for this card checkout. The server requests `SCA_WHEN_REQUIRED`, and the SDK handles any required 3-D Secure authentication before server capture.

Debit-card acceptance depends on the merchant's PayPal Advanced Credit and Debit Card Payments eligibility, supported card network/country and issuer approval. Cards must support online USD payments and, where applicable, cross-border transactions; domestic-only debit cards are not guaranteed to work. When `CardFields.isEligible()` is false, checkout explains that direct card payment is unavailable and offers PayPal. Before production acceptance, verify an eligible debit card through the provider's sandbox flow, including authentication, capture, entitlement and refund. `npm run test:payments` and `npm run check:checkout-ui` use mocked providers and do not establish live debit-card acceptance.
