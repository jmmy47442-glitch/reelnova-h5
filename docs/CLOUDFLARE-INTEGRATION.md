...
Checkout prepares the PayPal SDK and supported-device Apple Pay configuration on authenticated, unpurchased series pages. Public configuration and Apple Pay eligibility are reused for up to 60 seconds; preparation never creates orders or opens a wallet. The sheet immediately shows the three payment method buttons, even while public configuration is pending. Hosted card fields and PayPal buttons mount silently when the sheet opens; provider content and preparation errors appear only after the customer selects a method. Pending preparation shows a spinner and “Loading…” inside the selected button, preserving its size; other method buttons remain available. Reopening the sheet resets that selection. After 2.5 seconds of visible waiting for PayPal, customers can choose the server-created PayPal redirect without waiting for SDK timeout. Apple Pay connection failures can be retried in place.
