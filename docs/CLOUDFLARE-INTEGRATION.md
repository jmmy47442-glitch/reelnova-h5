...
Checkout prepares the PayPal SDK and supported-device Apple Pay configuration on authenticated, unpurchased series pages. Public configuration and Apple Pay eligibility are reused for up to 60 seconds; preparation never creates orders or opens a wallet. The sheet immediately shows the three payment method buttons, even while public configuration is pending. Hosted card fields and PayPal buttons mount silently when the sheet opens; provider content, progress and preparation errors appear only after the customer selects a method. Reopening the sheet resets that selection. After 2.5 seconds of visible waiting, checkout explains the delay. PayPal uses only its official SDK button: loading, the ready button and a retry state are mutually exclusive. Partially rendered SDK content stays hidden, and timeout offers “Retry PayPal” instead of a second payment action. Retry only prepares the provider and never creates an order. Apple Pay connection failures can be retried in place.
