...
export const useAdminApi = () => {
  ...
  return {
    ...
    refundOrder: (orderNo: string, reason: string, input: { amount?: string; method?: 'paypal_api' | 'manual' | 'reject'; providerStatus?: string; paypalRefundId?: string } = {}) => request<{ orderNo: string; paypalRefundId?: string; status: 'pending' | 'processing' | 'refunding' | 'refunded' | 'paid' | 'failed' | 'rejected'; synchronized: boolean }>(`/admin/orders/${orderNo}/refund`, { method: 'POST', body: { reason, ...input } }),
