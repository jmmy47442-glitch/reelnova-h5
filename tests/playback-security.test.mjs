...
test('rate limit buckets increment within a fixed window and record blocking metadata', () => {
  const database = createDatabase();
  const statement = database.prepare(`INSERT INTO playback_rate_limits
    (bucket_key, window_started_at, request_count, blocked_until, updated_at)
    VALUES ('bucket-1', ?, 1, NULL, ?)
  ...
  const consume = (windowStartedAt, now, limit = 2) => statement.get(
    windowStartedAt,
  ...
  assert.deepEqual({ ...consume(1000, 1000) }, { request_count: 1, blocked_until: null });
  assert.deepEqual({ ...consume(1000, 1001) }, { request_count: 2, blocked_until: null });
  assert.deepEqual({ ...consume(1000, 1002) }, { request_count: 3, blocked_until: 121002 });
  assert.deepEqual({ ...consume(1000, 1003) }, { request_count: 4, blocked_until: 121002 });
  consume(2000, 122000);
