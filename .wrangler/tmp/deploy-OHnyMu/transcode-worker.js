...
// workers/transcode-worker.mjs
import { WorkflowEntrypoint } from "cloudflare:workers";
var encoder = new TextEncoder();
var assetPattern = /^media_[0-9a-f-]{36}$/i;
var jobPattern = /^transcode_media_[0-9a-f-]{36}$/i;
var objectPattern = /^originals\/[a-z0-9_-]{2,100}\/[a-z0-9_-]{2,100}\/[a-z0-9_-]{2,100}\/[a-z0-9_.-]{2,160}$/i;
var bytesToHex = /* @__PURE__ */ __name((bytes) => Array.from(bytes, (byte) => byte.toString(16).padStart(2, "0")).join(""), "bytesToHex");
var hmac = /* @__PURE__ */ __name(async (value, secret) => {
...
var errorText = /* @__PURE__ */ __name((error) => (error instanceof Error ? error.message : "Video transcode failed").slice(0, 1e3), "errorText");
var notifyApplication = /* @__PURE__ */ __name(async (env, payload) => {
...
var validateJob = /* @__PURE__ */ __name((body) => {
  const job = {
  ...
  if (!jobPattern.test(job.jobId) || !assetPattern.test(job.assetId) || !objectPattern.test(job.sourceObjectKey) || !/^[a-zA-Z0-9_-]{8,160}$/.test(job.sourceEtag) || !/^[0-9a-f-]{36}$/i.test(job.buildId)) {
    throw new Error("Invalid transcode job request");
  }
  return job;
}, "validateJob");
var MediaTranscodeContainer = class extends Container {
  static {
    __name(this, "MediaTranscodeContainer");
  }
  defaultPort = 8080;
  requiredPorts = [8080];
  sleepAfter = "2m";
...
var MediaTranscodeWorkflow = class extends WorkflowEntrypoint {
  static {
    __name(this, "MediaTranscodeWorkflow");
  }
...
var startJob = /* @__PURE__ */ __name(async (env, body) => {
  ...
  try {
    ...
    try {
      instance = await env.MEDIA_TRANSCODE_WORKFLOW.get(job.jobId);
    } catch {
      throw error;
    }
...
export {
  MediaTranscodeContainer,
  MediaTranscodeWorkflow,
  transcode_worker_default as default
};
