...
interface ReturnOrder { paypal_order_id: string; series_slug: string; status: OrderStatus }

export default defineEventHandler(async (event) => {
  const query = getQuery(event);
  const orderNo = String(query.orderNo || '');
  const token = String(query.token || '');
  const order = await d1First<ReturnOrder>(event, 'SELECT paypal_order_id, series_slug, status FROM orders WHERE order_no = ?', [orderNo]);
