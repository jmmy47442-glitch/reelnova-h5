...
export default defineEventHandler(async (event) => {
  ...
  const order = await d1First<{ order_no: string }>(event,
    'SELECT order_no FROM orders WHERE paypal_order_id = ? AND user_id = ?', [paypalOrderId, session.userId]);
  if (!order) throw createError({ statusCode: 404, statusMessage: 'Order not found' });
  return ok(await cancelPayPalCheckout(event, paypalOrderId));
});
