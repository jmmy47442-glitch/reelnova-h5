...
export default defineEventHandler(async (event) => {
  ...
  await d1Run(event, `INSERT INTO orders
    (order_no, series_id, series_slug, series_title, visitor_id, country, amount_cents, currency, status, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, 'USD', 'pending', ?, ?)`, [orderNo, series.id, series.slug, series.title, visitorId, getRequestCountry(event), amountCents, now.toISOString(), now.toISOString()]);
  try {
