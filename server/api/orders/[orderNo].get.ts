...
interface OrderRow { order_no: string; series_id: string; series_title: string; amount_cents: number; currency: 'USD'; status: OrderStatus; created_at: string; updated_at: string; paypal_order_id: string | null; paypal_environment: PayPalEnvironment | null; payment_method: Order['paymentMethod']; refund_status: Order['refundStatus'] | null; entitlement_status: Order['entitlementStatus'] | null }
...
const selectOrder = (event: Parameters<typeof d1First>[0], orderNo: string, userId: string) => d1First<OrderRow>(event,
  `SELECT o.order_no, o.series_id, o.series_title, o.amount_cents, o.currency, o.status, o.created_at, o.updated_at,
    o.paypal_order_id, o.paypal_environment, o.payment_method,
...
export default defineEventHandler(async (event) => {
  ...
  if (['pending', 'processing'].includes(row.status) && row.paypal_order_id
    && Date.parse(row.updated_at) < Date.now() - 10_000) {
