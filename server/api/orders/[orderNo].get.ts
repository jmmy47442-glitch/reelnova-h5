...
export default defineEventHandler(async (event) => {
  ...
  const row = await d1First<OrderRow>(event, 'SELECT order_no, series_id, series_title, amount_cents, currency, status, created_at, paypal_order_id FROM orders WHERE order_no = ? AND user_id = ?', [orderNo, userSession.userId]);
